...
class SmoothReframer:
    def __init__(self, src_w: int, src_h: int, target_w: int, target_h: int, smooth_strength: float = 0.97, analysis_stride: int = 6, deadzone_ratio: float = 0.06, max_pan_ratio: float = 0.01):
...
def create_vertical_master(source_path: str, output_path: str, target_w: int = DEFAULT_TARGET_W, target_h: int = DEFAULT_TARGET_H, smooth_strength: float = 0.97, analysis_stride: int = 6, deadzone_ratio: float = 0.06, max_pan_ratio: float = 0.01, progress_cb: Optional[Callable[[float, str], None]] = None):
...
def build_realtime_rtmps_push_command(target_w: int, target_h: int, fps: float, rtmps_url: str, stream_key: str):
    ...
    return ["ffmpeg", "-y", "-f", "rawvideo", "-pix_fmt", "bgr24", "-s", f"{target_w}x{target_h}", "-r", str(fps_int), "-i", "-", "-f", "lavfi", "-i", "anullsrc=r=48000:cl=stereo", "-shortest", "-map", "0:v:0", "-map", "1:a:0", "-c:v", "libx264", "-preset", "veryfast", "-tune", "zerolatency", "-pix_fmt", "yuv420p", "-r", str(fps_int), "-b:v", "1500k", "-maxrate", "1700k", "-bufsize", "2500k", "-g", str(fps_int * 2), "-keyint_min", str(fps_int * 2), "-sc_threshold", "0", "-c:a", "aac", "-b:a", "128k", "-ar", "48000", "-ac", "2", "-f", "flv", target]
...
def _build_ingest_command(source: str, fps: float, pace_input: bool, loop_file: bool) -> list[str]:
    vf = f"fps={max(1, int(round(fps or PLACEHOLDER_FPS)))},scale={WORKING_INPUT_W}:{WORKING_INPUT_H}:force_original_aspect_ratio=decrease,pad={WORKING_INPUT_W}:{WORKING_INPUT_H}:(ow-iw)/2:(oh-ih)/2:black"
...
def _realtime_worker(session: LiveSession, source: str, target_w: int, target_h: int, delay_seconds: float, smooth_strength: float, analysis_stride: int, deadzone_ratio: float, max_pan_ratio: float, loop_file: bool, pace_input: bool) -> None:
    ...
    src_w = WORKING_INPUT_W; src_h = WORKING_INPUT_H
    frame_bytes = src_w * src_h * 3
    delay_frames = max(1, int(delay_seconds * fps))
    session.stats = {"fps": round(fps, 3), "delay_frames": delay_frames, "working_resolution": f"{src_w}x{src_h}", "source_reported_resolution": f"{int(info.get('width') or 0)}x{int(info.get('height') or 0)}"}
    reframer = SmoothReframer(src_w, src_h, target_w, target_h, smooth_strength, analysis_stride, deadzone_ratio, max_pan_ratio)
    buffer = collections.deque(maxlen=max(delay_frames + 240, 480))
    ...
    try:
        ...
        session.status = "ffmpeg_pipe_broken"; session.error = f"Could not prime Cloudflare output: {exc}"; return
    ingest = None
    try:
        ...
        next_deadline = time.time(); frames_in = 0; frames_out = 0; source_ended = False
        while not session.stop_event.is_set():
            ...
            if len(buffer) >= delay_frames:
                session.status = "streaming"; frame_to_write = buffer.popleft()
            elif not source_ended:
                session.status = "buffering"; frame_to_write = placeholder
            elif buffer:
                session.status = "draining"; frame_to_write = buffer.popleft()
            else:
                session.status = "source_ended"; break
            if session.proc and session.proc.stdin and frame_to_write is not None:
                try:
                    session.proc.stdin.write(frame_to_write.tobytes()); frames_out += 1
                except Exception as exc:
            ...
            if frames_out % int(max(1.0, fps)) == 0:
                session.stats.update({"frames_in": frames_in, "frames_out": frames_out, "buffer_len": len(buffer), "delay_seconds": round(delay_frames / max(fps, 1.0), 2)})
    except Exception as exc:
...
def start_realtime_delayed_vertical_push(cfg: CFStreamConfig, source: str, asset_name: str, target_w: int = DEFAULT_TARGET_W, target_h: int = DEFAULT_TARGET_H, delay_seconds: float = 20.0, smooth_strength: float = 0.97, analysis_stride: int = 6, deadzone_ratio: float = 0.06, max_pan_ratio: float = 0.01, loop_file: bool = False, pace_input: bool = True) -> LiveSession:
    live_input = create_live_input(cfg, name=safe_token(Path(asset_name).stem))
    uid = live_input["uid"]; rtmps_url = live_input["rtmps"]["url"]; stream_key = live_input["rtmps"]["streamKey"]
    hls_url, dash_url, iframe_url = build_public_playback_urls(cfg, uid)
